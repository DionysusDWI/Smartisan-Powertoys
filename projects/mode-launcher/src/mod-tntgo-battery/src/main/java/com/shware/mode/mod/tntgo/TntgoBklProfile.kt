...
class TntgoBklProfile(context: Context) {
    ...
    fun refreshCurve(force: Boolean = false) {
        val raw = samplesRaw()
        // ★★★★★ ㊲（2026-09-16）：新鲜度键 = **台账的全部内容**。
        //
        // ## 为什么不是「条数 ＋ 末条样本」
        //
        // 上一版用「条数 ＋ 末条样本」，治好了"只看条数"那一次
        // （台账环形装满后条数**恒为 [KEEP]**）。
        // 但它**仍有一个洞**：台账是**环形**的 ⇒ 当**新追加的样本与上一拍逐字相同**时，
        // 键**不变**，可环形**已经挤掉了最老一条** ⇒ **内容变了、键没变** ⇒ 提前 return。
        //
        // ⚠️ 这个洞**在真机上已经触发过**（2026-09-16 12:29，240 条台账里 1 处相邻逐字相同：
        //   `81:1000:4001:1`，充电段电流被充电器钉住），而当时缓冲**是混合的**（4 个档位）
        //   ⇒ 那一刻档位计数确实在漂，而曲线没有重算。
        // ⚠️ 影响**有界**：一次重复只丢**一拍**（30 s）重算，下一拍样本一变就自愈；
        //   但**连续逐字相同的游程有多长，陈旧窗口就有多长**。
        //
        // ⇒ 正解：键 = **全部内容**（条数已被内容蕴含，不再需要单独一项）。
        //   代价：每拍多一次约 3.5 KB 的拼接与比较 —— 30 s 一次，可忽略。
        val key = raw.joinToString(",")
        ...
        // ★ 记账的是**同一个 key**（台账全部内容）—— 改了节流键却忘了改这个 ⇒ 每一轮都重算。
        lastBuiltKey = key
    ...
    /**
     * 上次重算曲线时的**新鲜度键**（节流用）—— 形如 `"44:980:…,81:1000:-1507:0"`（**台账全部内容**）。
     *
     * ⚠️⛔ **不要改回"只存条数"**：台账是环形保留 [KEEP] 条 ⇒ 装满之后条数**恒定**，
     * 只比条数会让曲线**永不重算**（实测静默失效 8 分钟以上、无任何异常日志）。
     * ⚠️⛔ **也不要改成"条数 ＋ 末条样本"**：环形**每拍会挤掉一条老样本**，而末条相同时键不变
     * ⇒ 内容变了却判不出来（**真机已触发**，见 [refreshCurve] 的 ㊲ 注释）。
     * ★ 回归套件里有**反向守卫**盯着这两件事（④-⑨-B ／ ④-㊲）。
     */
